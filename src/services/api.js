...
export const api = {
  ...
  async getSamples(page = 1, pageSize = 20) {
    const response = await fetch(`${API_URL}/api/samples?page=${page}&page_size=${pageSize}`);
  ...
  async getParticles(sampleId, gridSize = 20) {
    const response = await fetch(`${API_URL}/api/weather/${sampleId}/particles?grid_size=${gridSize}`);
  ...
  async getAnimation(sampleId, start = 0, end = 10) {
    const response = await fetch(`${API_URL}/api/animation/${sampleId}?start=${start}&end=${end}`);
